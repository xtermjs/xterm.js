...
test.describe.only('WebGL Renderer Integration Tests', async () => {
  ...
  test('dispose removes renderer canvases', async function(): Promise<void> {
    assert.equal(await ctx.page.evaluate(`document.querySelectorAll('.xterm canvas').length`), 2);
    await ctx.page.evaluate(`addon.dispose()`);
    assert.equal(await ctx.page.evaluate(`document.querySelectorAll('.xterm canvas').length`), 0);
