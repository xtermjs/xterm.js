...
import { IBoundingBox, ICharAtlasConfig, IRasterizedGlyph, ITextureAtlas, ITextureAtlasDirtyRect } from './Types';
...
export class TextureAtlas implements ITextureAtlas {
  ...
  public get pages(): AtlasPage[] { return this._pages; }
  ...
  private _createNewPage(): AtlasPage {
    ...
    if (TextureAtlas.maxAtlasPages && this._pages.length >= Math.max(4, TextureAtlas.maxAtlasPages)) {
      ...
      mergedPage.invalidateDirtyHistory();
  ...
  private _deletePage(pageIndex: number): void {
    ...
    for (let j = pageIndex; j < this._pages.length; j++) {
      ...
      adjustingPage.invalidateDirtyHistory();
    }
  }
  ...
  private _evictAllPages(): void {
    ...
    this._didWarmUp = false;
    this._pageLayoutVersion++;
    this._logService.debug(`Evicted ${pageCount} WebGPU atlas pages in ${(performance.now() - startTime).toFixed(2)}ms`);
  }
  ...
  private _drawToCache(codeOrChars: number | string, bg: number, fg: number, ext: number, restrictToCellHeight: boolean, domContainer: HTMLElement | undefined): IRasterizedGlyph {
    ...
    activePage.markDirty(
      rasterizedGlyph.texturePosition.x,
      rasterizedGlyph.texturePosition.y,
      rasterizedGlyph.size.x,
      rasterizedGlyph.size.y
    );
...
export class AtlasPage {
  ...
  private _dirtyHistoryBaseVersion = this.version;
  private readonly _dirtyRects: ITextureAtlasDirtyRect[] = [];

  public getDirtyRectsSince(version: number): ReadonlyArray<ITextureAtlasDirtyRect> | undefined {
    if (version === this.version) {
      return [];
    }
    if (this._dirtyRects.length === 0 || version < this._dirtyHistoryBaseVersion) {
      return undefined;
    }
    return this._dirtyRects.filter(e => e.version > version);
  }
  ...
  public markDirty(x: number, y: number, width: number, height: number): void {
    ...
    let right = x + width;
    let bottom = y + height;
    for (let i = this._dirtyRects.length - 1; i >= 0; i--) {
      const rect = this._dirtyRects[i];
      const rectRight = rect.x + rect.width;
      const rectBottom = rect.y + rect.height;
      if (right < rect.x || rectRight < left || bottom < rect.y || rectBottom < top) {
        continue;
      }
      ...
      this._dirtyRects.splice(i, 1);
    }
    this._dirtyRects.push({ x: left, y: top, width: right - left, height: bottom - top, version: this.version });
    // Bound retained history so a shared atlas cannot grow indefinitely when a
    // renderer is hidden. Lagging consumers safely fall back to a full upload.
    if (this._dirtyRects.length > 256) {
      const removed = this._dirtyRects.splice(0, this._dirtyRects.length - 256);
      this._dirtyHistoryBaseVersion = removed[removed.length - 1].version;
    }
  }

  public invalidateDirtyHistory(): void {
    this._dirtyRects.length = 0;
    this.version = ++AtlasPage.nextVersion;
    this._dirtyHistoryBaseVersion = this.version;
  }
