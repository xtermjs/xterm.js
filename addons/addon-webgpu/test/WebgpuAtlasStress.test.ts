...
import type { IWebgpuAddonOptions, WebgpuAddon } from '@xterm/addon-webgpu';
...
type TestWebgpuAddonConstructor = new (options?: IWebgpuAddonOptions) => ITestWebgpuAddon;
...
interface ITestWebgpuAddon extends WebgpuAddon {
...
interface ITestWindow {
  WebgpuAddon: TestWebgpuAddonConstructor;
  term: ITestTerminal;
  addon?: ITestWebgpuAddon;
...
async function loadWebgpuStrict(ctx: ITestContext): Promise<void> {
  await ctx.page.evaluate(async () => {
    const w = window as unknown as ITestWindow;
    w.addon = new w.WebgpuAddon({});
    w.term.loadAddon(w.addon);
    await w.addon.ready;
  });
  const isWebgpuRenderer = await ctx.page.evaluate(() => {
    const w = window as unknown as ITestWindow;
    return !!w.addon && w.term._core?._renderService?._renderer?.value === w.addon._renderer;
  });
  expect(isWebgpuRenderer, 'WebGPU renderer must be active').toBe(true);
}
...
test.describe('WebGPU atlas TUI stress (#6038)', () => {
  test.skip(({ browserName }) => browserName !== 'chromium', 'WebGPU atlas tests run on Chromium/SwiftShader only');
