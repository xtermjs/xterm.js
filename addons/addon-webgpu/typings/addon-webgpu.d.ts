...
declare module '@xterm/addon-webgpu' {
  /**
   * An xterm.js addon that provides hardware-accelerated rendering functionality via WebGPU.
   */
  export class WebgpuAddon implements ITerminalAddon {
    public textureAtlas?: HTMLCanvasElement;

    /**
     * Resolves after the WebGPU renderer has been installed. The promise rejects
     * when WebGPU initialization fails or the addon is disposed first.
     */
    public readonly ready: Promise<void>;

    /** An event that is fired when renderer initialization or rendering fails. */
    public readonly onRendererError: IEvent<Error>;

    /** An event that is fired when the WebGPU device is lost unexpectedly. */
    public readonly onDeviceLoss: IEvent<IWebgpuDeviceLostEvent>;
  ...
  /** Stable device-loss information exposed without browser experimental types. */
