...
export class ImageRenderer extends Disposable implements IDisposable {
  ...
  /**
   * Current cell size (float), in DEVICE pixels. Image layers render at device
   * resolution so images stay crisp on HiDPI displays; the decoders rasterize
   * their sources at the same resolution (see `dpr`).
   */
  ...
  /**
   * Device-pixel ratio of the terminal's window. Decoders use it to rasterize
   * CSS-sized images at device resolution so they stay crisp on HiDPI displays.
   * Uses the true devicePixelRatio (exactly 1 on non-HiDPI displays) rather
   * than a cell-size ratio, so that at dpr 1 no resampling happens at all and
   * pixel-exact sources are preserved. Falls back to 1 when unavailable.
   */
  public get dpr(): number {
    return this._terminal._core._coreBrowserService?.dpr ?? 1;
  }
  ...
  public clearLines(start: number, end: number, layer?: ImageLayer): void {
    ...
    const h = (end + 1 - start) * (this.dimensions?.device.cell.height || 0);
    if (!layer || layer === 'top') {
