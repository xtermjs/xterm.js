 ...
 */
interface Uint8ArrayExt extends Uint8Array {
  toBase64(): string;
}
interface Uint8ArrayExtCtor extends Uint8ArrayConstructor {
  fromBase64(s: string): Uint8ArrayExt;
}

export class Base64 implements IBase64 {
  public encodeText(data: string): string {
    const bytes = new TextEncoder().encode(data) as Uint8ArrayExt;
  ...
  public decodeText(data: string): string {
    if ((Uint8Array as Uint8ArrayExtCtor).fromBase64 !== undefined) {
      try {
        return new TextDecoder().decode((Uint8Array as Uint8ArrayExtCtor).fromBase64(data));
      } catch {}
      return '';
    }
    try {
