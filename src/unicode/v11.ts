...
const ZERO_WIDTH_BMP = [
  ...
  [0x0951, 0x0957],  // Devanagari Stress Sign U..
  ...
  [0x0f8d, 0x0f97],  // (nil)                   ..Tibetan Subjoined Letter
  ...
  [0x135d, 0x135f],  // (nil)                   ..Ethiopic Combining Gemin
  ...
  [0x1cf4, 0x1cf4],  // (nil)                   ..
  [0x1cf8, 0x1cf9],  // (nil)                   ..
  [0x1dc0, 0x1df9],  // Combining Dotted Grave A..
  ...
  [0xa674, 0xa67d],  // (nil)                   ..Combining Cyrillic Payer
  [0xa69e, 0xa69f],  // (nil)                   ..
...
/*
  Ideally the following values should be used to create the lookup table,
  needs to be tested before we activate this.
...
const WIDE_EASTASIAN_BMP = [
  ...
  [0x3190, 0x31ba],  // Ideographic Annotation L..
...
function wcwidthHigh(ucs: number): 0 | 1 | 2 {
  if (bisearch(ucs, ZERO_WIDTH_HIGH)) {
    return 0;
  }
  if (bisearch(ucs, WIDE_EASTASIAN_HIGH)) {
    return 2;
  }
  return 1;
}
...
export const v11: IUnicodeVersionProvider = {
  version: 11,
  wcwidth: (ucs: number): number => {
    ...
    if (ucs < 65536) {
      return lookupTable[ucs];
    }
    // do a full search for high codepoints
    return wcwidthHigh(ucs);
  },
  init: () => {
    // init only once
    if (lookupTable) {
    ...
    // FIXME: after testing and use WIDE_EASTASIAN_BMP below instead
    ...
    // for (let r = 0; r < WIDE_EASTASIAN_BMP.length; ++r) {
    //   fill(table, 2, WIDE_EASTASIAN_BMP[r][0], WIDE_EASTASIAN_BMP[r][1] + 1);
    // }
    ...
    // combining 0
    for (let r = 0; r < ZERO_WIDTH_BMP.length; ++r) {
      fill(table, 0, ZERO_WIDTH_BMP[r][0], ZERO_WIDTH_BMP[r][1] + 1);
    }
